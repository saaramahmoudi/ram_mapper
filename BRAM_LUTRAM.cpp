...
void get_memory_utilization(){  
    for(int i = 0 ;  i < num_of_circuits; i++){
        ...
        // cout << "TOTAL NUMBER OF BRAM " << ceil((double)LUT_in_circuit[i]/BRAM_RATIO) << " ,USED 8KBRAM : " << circuits[i].num_of_BRAM << endl;
        ...
        // cout << "BRAM UTILIZATION " << circuits[i].util_BRAM << endl;    
...
void update_LUT_number(int c_index){
    int lut_by_BRAM = circuits[c_index].num_of_BRAM * BRAM_RATIO;
    ...
    if(lut_block >= lut_by_BRAM){
        ...
        LUT_in_circuit[c_index] = lut_by_BRAM;
    }

    circuits[c_index].util_LUT = (double) (circuits[c_index].num_of_LUTRAM + circuits[c_index].num_of_logic)/LUT_in_circuit[c_index]; 
    circuits[c_index].util_BRAM = (double) (circuits[c_index].num_of_BRAM/ (ceil((double) LUT_in_circuit[c_index] / BRAM_RATIO)));         
...
void find_best_mapping(RAM r, int circuit_id, bool re_map){
    ...
    if(LUTRAM_fit.req_area > BRAM_fit.req_area){
        ...
        best_p = LUTRAM_fit.p;
        best_s = LUTRAM_fit.s;
        circuits[circuit_id].num_of_LUTRAM += best_s*best_p;
    }
...
void pack_leftovers(){
    for(int i = 0; i < lp_maps.size(); i++){
        ...
        if(lp_maps[i].type == 1 && lp_maps[i].mode == BRAM_MAX_WIDTH)
            continue;
...
//move from LUTRAM to BRAM
void balance_LUTRAM_to_BRAM(){
    //loop over all circuits
    for(int i = 0; i < num_of_circuits; i++){
        if(circuits[i].util_LUT <= circuits[i].util_BRAM)
            continue;
        ...
        update_LUT_number(i);
        int avail_BRAM   = ceil((double)LUT_in_circuit[i]/BRAM_RATIO) - circuits[i].num_of_BRAM;
    
        //loop over the candidates 
        for(int j = 0; j < LUTRAMS.size(); j++){
            //if nothing left to fill break the process
            if(circuits[i].num_of_LUTRAM <= 0 || avail_BRAM <= 0)
                break;

            int moving_r_index = LUTRAMS[j].r_index;

            Fit BRAM_fit = best_mapping_on_BRAM(circuits[i].ram[moving_r_index],true);
            
            if(BRAM_fit.s * BRAM_fit.p <= avail_BRAM){
                //increase the number of BRAM8K
                circuits[i].num_of_BRAM += BRAM_fit.s * BRAM_fit.p;
                //reducing the number of LUTRAM
                circuits[i].num_of_LUTRAM -= LUTRAMS[j].s * LUTRAMS[j].p;
                
                //update since number of resources changed
                update_LUT_number(i);
                avail_BRAM   = ceil((double)(LUT_in_circuit[i]/BRAM_RATIO)) - circuits[i].num_of_BRAM;

                if(avail_BRAM >= 0 && circuits[i].util_BRAM < circuits[i].util_LUT){
                    //moving process - update the mapping structure
                    int lp_index = circuits[i].ram[moving_r_index].lp_map_id;
                    lp_maps[lp_index].type = BRAM_fit.type;
                    lp_maps[lp_index].mode = BRAM_fit.mode;
                    lp_maps[lp_index].s = BRAM_fit.s;
                    lp_maps[lp_index].p = BRAM_fit.p;
                    lp_maps[lp_index].additional_logic = BRAM_fit.req_logic;
                    continue;
                }
                else{
...
void balance_BRAM(){
    ...
    for(int i = 0; i < num_of_circuits; i++){
        ...
        if(circuits[i].util_BRAM <= circuits[i].util_LUT)
            continue;

        balance[i] = true;

        //get the logical RAMs mapped to 8KBRAM and sort them
        vector<LP_map> BRAMS = get_BRAMS(i,1);
        sort(BRAMS.begin(),BRAMS.end(),compare_two_Logical_RAM_ascending);
        
        //calculate available resources
        int avail_logic = ceil(LUT_in_circuit[i] / LUTRAM_RATIO) - circuits[i].num_of_LUTRAM;
        
        for(int j = 0; j < BRAMS.size(); j++){    
            //no need to move anymore
            if(circuits[i].util_LUT >= circuits[i].util_BRAM)
                break;
            
        
            int moving_r_index = BRAMS[j].r_index;
            ...
            if(circuits[i].ram[BRAMS[j].r_index].mode == "TrueDualPort" || LUTRAM_fit.req_area == -1){
               move =  0;//can not be mapped to LUTRAM
            }
            else if(circuits[i].util_LUT < circuits[i].util_BRAM){
            ...
            if(move == 1){//move to LUTRAM
                if(LUTRAM_fit.s * LUTRAM_fit.p <= avail_logic && LUTRAM_fit.s * LUTRAM_fit.p != 0){
                    ...
                    circuits[i].num_of_BRAM -= BRAMS[j].s * BRAMS[j].p;
