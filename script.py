...
while mem_size <= 2**17:
    ...
    while max_width <= mem_size/256:
        ...
        while ratio <= 300:
            ...
            checker_res = subprocess.run(["./checker",f"-b", f"{mem_size}", f"{result[0]}", f"{result[1]}", "1", "logical_rams.txt", "logic_block_count.txt", "checker_input.txt"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            result_check = (checker_res.stdout[:-1]).decode("utf-8")
            result_check = result_check.split()
            # print(result_check[247])
            area = float(result_check[247])
            if(best_area == -1 or best_area > area):
                best_area = area
                best_max_width = max_width
                best_ratio = ratio
                best_area_sci_notation = result_check[247]
            if ratio == 1:
                ratio += 4
            else:
                ratio += 5
